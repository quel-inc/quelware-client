...
def test_set_fixed_timeline_roundtrip():
    original = SetFixedTimeline(
        waveform_library=[
            IqWaveform(
                sampling_period_fs=1000, 
                iq_array=np.array([1.0 + 2.0j, -0.5 - 1.5j])
            )
        ],
        events=[
            WaveformEvent(
    ...
    np.testing.assert_array_equal(
        recovered.waveform_library[0].iq_array,
        original.waveform_library[0].iq_array
    )
